...
function selectAllColors() {
  const n = parseInt(el("numBottles").value, 10);
  if (n !== 14) return;
  el("colorChecklist").querySelectorAll("input[type=checkbox]").forEach(cb => cb.checked = true);
  updateColorLimitUI();
...
function onLayerClick(b, l) {
  ...
  // tap same selected layer => CLEAR (still available even without eraser)
...
function renderPopover(b) {
  ...
  for (const c of colors) {
    // true remaining = 4 - usedCount
    const remaining = CAP - (counts[c] || 0);

    // Hide completely at 0 remaining
    ...
    box.addEventListener("click", () => {
      ...
      // auto-advance within the same bottle
      const next = findNextEmptyLayerInBottle(selectedLayer.b, selectedLayer.l);
      if (next !== null) {
        selectedLayer = { b: selectedLayer.b, l: next };
        openPopoverBottle = selectedLayer.b;
        renderAllLayers();
        renderPopover(selectedLayer.b);
      } else {
        renderAllLayers();
        renderPopover(selectedLayer.b);
      }
...
function findNextEmptyLayerInBottle(b, startL) {
  for (let l = startL + 1; l < CAP; l++) if (!bottleLayers[b][l]) return l;
  for (let l = 0; l <= startL; l++) if (!bottleLayers[b][l]) return l;
  return null;
}
...
function setLayerColor(b, l, color, pushHistory=false) {
  ...
  renderAllLayers();
  if (openPopoverBottle !== null) renderPopover(openPopoverBottle);
}
...
function onIOApply() {
  ...
  if (mode === "export") { el("ioMsg").textContent = "Copy the code above."; return; }
  try {
