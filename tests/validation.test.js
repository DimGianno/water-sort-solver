...
function createFixture({ checkedColors = ["Red", "Blue"], layers = [] } = {}) {
  const checkboxes = DEFAULT_COLORS.map((value) => ({ value, checked: checkedColors.includes(value) }));
  const elements = {
    colorChecklist: {
  ...
  const state = { bottleLayers: layers.map((bottle) => bottle.slice()) };
...
test("validateInput reports structural and color-count errors", async (t) => {
  ...
  await t.test("requires selected colors", () => {
    assert.equal(validation.validateInput([[], [], [], []], []), "Select colors first.");
  });

  await t.test("requires at least four bottles", () => {
    assert.equal(validation.validateInput([[], [], []], ["Red"]), "Invalid bottle count (min 4).");
  });

  await t.test("requires both helper bottles to be empty", () => {
    const bottles = [["Red", "Red", "Red", "Red"], ["Blue", "Blue", "Blue", "Blue"], ["Red"], []];
    assert.equal(validation.validateInput(bottles), "Last 2 bottles must be empty (helpers).");
  });

  await t.test("requires every playable bottle to be full", () => {
    const bottles = [["Red", "Red", "Red"], ["Blue", "Blue", "Blue", "Blue"], [], []];
    assert.equal(validation.validateInput(bottles), `Bottle 1 must have exactly ${CAP} layers (fill all).`);
  });

  await t.test("rejects colors that were not selected", () => {
    const bottles = [["Red", "Red", "Red", "Green"], ["Blue", "Blue", "Blue", "Blue"], [], []];
    assert.equal(validation.validateInput(bottles), 'Color "Green" is used but not selected in the checklist.');
  });

  await t.test("requires exactly four pieces of each selected color", () => {
    const bottles = [["Red", "Red", "Red", "Red"], ["Blue", "Blue", "Blue", "Red"], [], []];
    assert.equal(validation.validateInput(bottles), 'Color "Red" appears 5 times, but must appear exactly 4 times.');
  });
});
...
test("continuous validation keeps the message and solve button in sync", () => {
  ...
  assert.equal(elements.validationMsg.textContent, "Input looks valid. You can solve.");
