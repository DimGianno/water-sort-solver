...
import { fillSolvablePuzzle, openFourBottleBuilder, paletteColor } from "./helpers.js";

test("layer-first entry restores inventory when a layer is cleared", async ({ page }) => {
  await openFourBottleBuilder(page);

  await paletteColor(page, "Red").click();
  await expect(page.getByRole("button", { name: "Bottle 1, layer 1, Red" })).toBeVisible();
  await expect(paletteColor(page, "Red")).toHaveAccessibleName("Red, 3 remaining");

  await page.getByRole("button", { name: "Bottle 1, layer 1, Red" }).click();
  await page.getByRole("button", { name: "Clear layer" }).click();

  await expect(page.getByRole("button", { name: "Bottle 1, layer 1, empty" })).toBeVisible();
  await expect(paletteColor(page, "Red")).toHaveAccessibleName("Red, 4 remaining");
});
...
test("color-first entry paints selected layers and enforces inventory", async ({ page }) => {
  await openFourBottleBuilder(page);
  await page.locator("label", { hasText: "Color first" }).click();

  await paletteColor(page, "Red").click();
  await expect(paletteColor(page, "Red")).toHaveAttribute("aria-pressed", "true");
  for (const layer of [
